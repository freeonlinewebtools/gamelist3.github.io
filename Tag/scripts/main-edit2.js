...
window.DOMElementHandler = class extends self.DOMHandler {
    constructor(a, b) {
        ...
        this._autoAttach = !0;
        this.AddRuntimeMessageHandlers([["create", c => this._OnCreate(c)], ["destroy", c => this._OnDestroy(c)], ["set-visible", c => this._OnSetVisible(c)], ["update-position", c => this._OnUpdatePosition(c)], ["update-state", c => this._OnUpdateState(c)], ["focus", c => this._OnSetFocus(c)], ["set-css-style", c => this._OnSetCssStyle(c)], ["set-attribute", c => this._OnSetAttribute(c)], ["remove-attribute", c => this._OnRemoveAttribute(c)]]);
...
window.RuntimeInterface = class a {
    constructor(b) {
        ...
        if ("playable-ad" === this._exportType || "instant-games" === this._exportType)
            this._useWorker = !1;
    ...
    async _Init(b) {
        ...
        b.runtimeBaseUrl ? this._runtimeBaseUrl = b.runtimeBaseUrl : (c = location.origin,
        this._runtimeBaseUrl = ("null" === c ? "file:///" : c) + location.pathname,
        c = this._runtimeBaseUrl.lastIndexOf("/"),
        -1 !== c && (this._runtimeBaseUrl = this._runtimeBaseUrl.substr(0, c + 1)));
    ...
    _GetWorkerURL(b) {
        b = this._workerScriptURLs.hasOwnProperty(b) ? this._workerScriptURLs[b] : b.endsWith("/workermain.js") && this._workerScriptURLs.hasOwnProperty("workermain.js") ? this._workerScriptURLs["workermain.js"] : "playable-ad" === this._exportType && this._localFileBlobs.hasOwnProperty(b) ? this._localFileBlobs[b] : b;
        ...
        if ("cordova" === this._exportType)
            for (let h = 0, k = b.projectScripts.length; h < k; ++h) {
                const l = b.projectScripts[h]
                  , m = l[0];
                if (m === b.mainProjectScript || "scriptsInEvents.js" === m || m.endsWith("/scriptsInEvents.js"))
    ...
    async _InitDOM(b, c) {
        ...
        for (let[g,h] of f)
            ...
            g === e)
                try {
                    ...
                    await AddScript(h),
                    "preview" !== this._exportType || d[g] || this._ReportProjectMainScriptError(g, "main script did not run to completion")
                } catch (k) {
        ...
        alert("Failed to load JavaScript code used in events. Check all your JavaScript code has valid syntax.")) : (b = Object.assign(this._GetCommonRuntimeOptions(b), {
            isInWorker: !1,
            messagePort: c,
            canvas: this._canvas,
    ...
    _OnMessageFromRuntime(b) {
        const c = b.type;
        if ("event" === c)
            return this._OnEventFromRuntime(b);
        if ("result" === c)
            this._OnResultFromRuntime(b);
    ...
    _OnRAFCallback() {
        this._rafId = -1;
        for (const b of this._rafCallbacks)
            b();
        this._RequestAnimationFrame()
    }
    TryPlayMedia(b) {
    ...
    async _WasmDecodeWebMOpus(b) {
        b = await this.PostToRuntimeComponentAsync("runtime", "opus-decode", {
            arrayBuffer: b
        }, null, [b]);
    ...
    async _MaybeGetCordovaScriptURL(b) {
        return "cordova" === this._exportType && (b.startsWith("file:") || this._isFileProtocol && this.IsRelativeURL(b)) ? (b.startsWith(this._runtimeBaseUrl) && (b = b.substr(this._runtimeBaseUrl.length)),
    ...
    async RequestPermissions(b) {
        if ("cordova" !== this.GetExportType() || this.IsiOSCordova())
            return !0;
        for (const c of b)
            if (!await this._HasPermission(c) && !1 === await this._RequestPermission(c))
                return !1;
        return !0
    }
    ...
    async _ConvertDataUriToBlobs(b, c) {
        if ("object" === typeof c)
            this._localFileBlobs[b] = new Blob([c.str],{
                type: c.type
            }),
    ...
    _BinaryStringToBlob(b, c) {
        ...
        for (k = h = 0; h < e; ++h)
            g[h] = b.charCodeAt(k++) | b.charCodeAt(k++) << 8 | b.charCodeAt(k++) << 16 | b.charCodeAt(k++) << 24;
        for (d &= 3; d--; )
            f[k] = b.charCodeAt(k),
            ++k;
        return new Blob([f],{
...
function IsInContentEditable(a) {
    do {
        if (a.parentNode && a.hasAttribute("contenteditable"))
            return !0;
        a = a.parentNode
    } while (a);
    return !1
...
RuntimeInterface$jscomp$1.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    Attach() {
        ...
        window.addEventListener("pointerrawupdate", b => this._OnPointerRawUpdate(b))) : window.addEventListener("pointermove", b => this._OnPointerEvent("pointermove", b)),
        window.addEventListener("pointerup", b => this._OnPointerEvent("pointerup", b)),
        window.addEventListener("pointercancel", b => this._OnPointerEvent("pointercancel", b))) : (window.addEventListener("mousedown", b => {
            this._HandlePointerDownFocus(b);
        ...
        if (!this._isExportToVideo && this._enableWindowResizeEvent) {
            ...
            if (this._iRuntime.IsAndroidWebView()) {
                if (this._enableAndroidVKDetection) {
                    ...
                    0 < this._virtualKeyboardHeight && (this._virtualKeyboardHeight = 0,
                    this._OnAndroidVirtualKeyboardChange(b, this._virtualKeyboardHeight))
                }
    ...
    _TrySetTargetOrientation() {
        ...
        if (screen.orientation && screen.orientation.lock)
            screen.orientation.lock(a).catch(b => console.warn("[Construct] Failed to lock orientation: ", b));
        else
            try {
                let b = !1;
    ...
    _OnMouseWheelEvent(a, b) {
        this._isExportToVideo || this.PostToRuntime(a, {
            clientX: b.clientX,
    ...
    _OnPointerEvent(a, b) {
        if (!this._isExportToVideo) {
            ...
            this._pointerRawMovementY = this._pointerRawMovementX = 0;
            "mouse" === b.pointerType && (c = "mousemove",
            "pointerdown" === a ? c = "mousedown" : "pointerup" === a && (c = "mouseup"),
            this._OnMouseEvent(c, b, DISPATCH_SCRIPT_ONLY),
            this._mousePointerLastButtons = b.buttons)
        }
    }
    _OnPointerRawUpdate(a) {
        this._lastPointerRawUpdateEvent && (this._pointerRawMovementX += this._lastPointerRawUpdateEvent.movementX || 0,
    ...
    _AttachDeviceMotionEvent() {
        ...
        window.addEventListener("devicemotion", a => this._OnDeviceMotion(a)))
    }
    ...
    _OnUpdateCanvasSize(a) {
        ...
        b.IsExportingToVideo() || (b = b.GetCanvas(),
        ...
        document.documentElement.style.setProperty("--construct-scale", a.displayScale),
    ...
    _OnPostToDebugger(a) {
        window.c3_postToMessagePort && (a.from = "runtime",
        window.c3_postToMessagePort(a))
    }
    ...
    _OnWrapperMessage(a) {
        "entered-fullscreen" === a ? (RuntimeInterface$jscomp$1._SetWrapperIsFullscreenFlag(!0),
        this._OnFullscreenChange()) : "exited-fullscreen" === a ? (RuntimeInterface$jscomp$1._SetWrapperIsFullscreenFlag(!1),
        this._OnFullscreenChange()) : console.warn("Unknown wrapper message: ", a)
    }
...
self.JobSchedulerDOM = class {
    ...
    async _CreateJobWorker() {
        ...
        var b = this._runtimeInterface._GetWorkerURL(this._GetWorkerScriptFolder() + "jobworker.js");
...
self.AudioDOMHandler = class extends self.DOMHandler {
    ...
    _ReconnectEffects(a) {
        ...
        for (const d of this.audioInstancesByTag(a))
            d.Reconnect(b);
    ...
    *audioInstancesByTag(a) {
    ...
    async _GetAudioInstance(a, b, c, d, e) {
        for (const f of this._audioInstances)
            if (f.GetUrl() === b && (f.CanBeRecycled() || e))
                return f.SetTag(d),
                f;
    ...
    async _Play(a) {
        const b = a.originalUrl
          ...
          , m = a.stereoPan;
        let n = a.off;
        0 < n && !a.trueClock && (this._audioContext.getOutputTimestamp ? (a = this._audioContext.getOutputTimestamp(),
        n = n - a.performanceTime / 1E3 + a.contextTime) : n = n - performance.now() / 1E3 + this._audioContext.currentTime);
        ...
        try {
            ...
            this._lastAudioInstance.SetPan(l.x, l.y, l.angle, l.innerAngle, l.outerAngle, l.outerGain),
            l.hasOwnProperty("uid") && this._lastAudioInstance.SetUID(l.uid)) : "number" === typeof m && 0 !== m ? (this._lastAudioInstance.SetStereoPannerEnabled(!0),
            this._lastAudioInstance.SetStereoPan(m)) : (this._lastAudioInstance.SetPannerEnabled(!1),
    ...
    _SetPaused(a) {
        const b = a.tag;
        a = a.paused;
        for (const c of this.audioInstancesByTag(b))
            a ? c.Pause() : c.Resume();
    ...
    async _FadeVolume(a) {
        ...
        for (const e of this.audioInstancesByTag(b))
            e.FadeVolume(c, d, a);
        this._MaybeStartTicking()
    }
    _SetMasterVolume(a) {
    ...
    _OnTick(a) {
        ...
        !(b = a.listenerPos) || this._lastListenerPos[0] === b[0] && this._lastListenerPos[1] === b[1] && this._lastListenerPos[2] === b[2] || (this._lastListenerPos[0] = b[0],
        this._lastListenerPos[1] = b[1],
        this._lastListenerPos[2] = b[2],
        this._audioContext.listener.setPosition(b[0], b[1], b[2]));
        for (const c of a.instPans) {
            ...
            for (const d of this._audioInstances)
                d.GetUID() === a && d.SetPanXYA(c.x, c.y, c.angle)
        }
    }
    async _AddEffect(a) {
        var b = a.type;
        ...
        var d = a.params;
        if ("filter" === b)
            d = new self.C3AudioFilterFX(this,...d);
        else if ("delay" === b)
            d = new self.C3AudioDelayFX(this,...d);
            ...
            d = new self.C3AudioFlangerFX(this,...d);
        else if ("phaser" === b)
            d = new self.C3AudioPhaserFX(this,...d);
        else if ("gain" === b)
            d = new self.C3AudioGainFX(this,...d);
        else if ("stereopan" === b)
            d = new self.C3AudioStereoPanFX(this,...d);
        else if ("tremolo" === b)
            d = new self.C3AudioTremoloFX(this,...d);
        else if ("ringmod" === b)
            d = new self.C3AudioRingModFX(this,...d);
        else if ("distortion" === b)
            d = new self.C3AudioDistortionFX(this,...d);
        else if ("compressor" === b)
            d = new self.C3AudioCompressorFX(this,...d);
        else if ("analyser" === b)
            d = new self.C3AudioAnalyserFX(this,...d);
        else
    ...
    _SetEffectParam(a) {
        ...
        !a || 0 > b || b >= a.length || (a[b].SetParam(c, d, e, f),
        this._PostUpdatedFxState())
    }
    _RemoveEffects(a) {
        a = a.tag.toLowerCase();
    ...
    _DoPostUpdatedFxState() {
        const a = {};
        for (const [b,c] of this._effects)
            a[b] = c.map(d => d.GetState());
    ...
    async _OnLoadState(a) {
        ...
        if (3 !== b) {
            var c = [];
            for (const d of this._audioInstances)
                d.IsMusic() && 1 === b || !d.IsMusic() && 2 === b ? c.push(d) : d.Release();
            this._audioInstances = c
        }
        for (const d of this._effects.values())
            for (const e of d)
                e.Release();
        this._effects.clear();
        this._timeScale = a.timeScale;
        ...
        this._destinationNode.gain.value = this._masterVolume;
        c = [];
        for (const d of Object.values(a.effects))
            c.push(Promise.all(d.map(e => this._AddEffect(e))));
        await Promise.all(c);
    ...
    async _OnOfflineRenderAudio(a) {
        try {
            ...
            this._hasStartedOfflineRender ? this._audioContext.resume() : (this._audioContext.startRendering().then(c => this._OnOfflineRenderCompleted(c)).catch(c => this._OnOfflineRenderError(c)),
...
self.C3WebAudioBuffer = class extends self.C3AudioBuffer {
    ...
    async _Fetch() {
        ...
        if ("cordova" === a.GetExportType() && a.IsRelativeURL(this._url) && a.IsFileProtocol())
...
self.C3AudioInstance = class {
    ...
    SetPannerEnabled(a) {
        a = !!a;
        this._isPannerEnabled !== a && ((this._isPannerEnabled = a) ? (this.SetStereoPannerEnabled(!1),
    ...
    SetPan(a, b, c, d, e, f) {
        this._isPannerEnabled && (this.SetPanXYA(a, b, c),
        a = self.AudioDOMHandler.ToDegrees,
        this._pannerConeParams[0] !== a(d) && (this._pannerConeParams[0] = a(d),
        this._pannerNode.coneInnerAngle = a(d)),
        this._pannerConeParams[1] !== a(e) && (this._pannerConeParams[1] = a(e),
        this._pannerNode.coneOuterAngle = a(e)),
        this._pannerConeParams[2] !== f && (this._pannerConeParams[2] = f,
        this._pannerNode.coneOuterGain = f))
    }
    SetPanXYA(a, b, c) {
        if (this._isPannerEnabled) {
            var d = this._pannerPosition
              , e = this._pannerOrientation
              , f = Math.cos(c);
            c = Math.sin(c);
            if (d[0] !== a || d[1] !== b || 0 !== d[2])
                d[0] = a,
                d[1] = b,
                d[2] = 0,
                this._pannerNode.setPosition(...d);
            if (e[0] !== f || e[1] !== c || 0 !== e[2])
                e[0] = f,
                e[1] = c,
                e[2] = 0,
                this._pannerNode.setOrientation(...e)
        }
    }
    SetStereoPannerEnabled(a) {
        a = !!a;
        this._isStereoPannerEnabled !== a && ((this._isStereoPannerEnabled = a) ? (this.SetPannerEnabled(!1),
    ...
    LoadStereoPanState(a) {
        "number" !== typeof a ? this.SetStereoPannerEnabled(!1) : (this.SetStereoPannerEnabled(!0),
        this.SetStereoPan(a))
    }
}
;
...
self.C3Html5AudioInstance = class extends self.C3AudioInstance {
    ...
    SetLooping(a) {
        a = !!a;
        this._isLooping !== a && (this._isLooping = a,
        this.GetAudioElement().loop = a)
    }
    _UpdatePlaybackRate() {
    ...
    SetSuspended(a) {
        a ? this.IsPlaying() ? (this.GetAudioElement().pause(),
...
self.C3WebAudioInstance = class extends self.C3AudioInstance {
    ...
    _OnEnded(a) {
        this._isPaused || this._resumeMe || a.target !== this._activeSource || (this._isStopped = this._hasPlaybackEnded = !0,
        this._instUid = -1,
        this._ReleaseBufferSource(),
        this._audioDomHandler.PostTrigger("ended", this._tag, this._aiId))
    }
    ...
    Seek(a) {
        this._isStopped || this.HasEnded() || (this._isPaused ? this._resumePosition = a : (this.Pause(),
    ...
    SetSuspended(a) {
        a ? this.IsPlaying() ? (this._resumeMe = !0,
...
class AudioFXBase {
    ...
    SetAudioParam(a, b, c, d) {
        ...
        if (0 === d)
            ...
            case 2:
                a.setValueAtTime(a.value, e),
                a.exponentialRampToValueAtTime(b, d)
            }
        }
    }
    GetState() {
...
self.C3AudioStereoPanFX = class extends AudioFXBase {
    ...
    SetParam(a, b, c, d) {
        ...
        case 9:
            this._params[0] = b,
            this.SetAudioParam(this._node.pan, b, c, d)
        }
    }
}
;
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    async _RequestOrientationPermission() {
        ...
        try {
            ...
            return console.warn("[Touch] Failed to request orientation permission: ", a),
            !1
        }
    }
    async _RequestMotionPermission() {
        ...
        try {
            ...
            return console.warn("[Touch] Failed to request motion permission: ", a),
            !1
        }
    }
}
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    constructor(a) {
        super(a, "mouse");
        this.AddRuntimeMessageHandlers([["cursor", b => this._OnChangeCursorStyle(b)], ["request-pointer-lock", () => this._OnRequestPointerLock()], ["release-pointer-lock", () => this._OnReleasePointerLock()]]);
        document.addEventListener("pointerlockchange", b => this._OnPointerLockChange());
        document.addEventListener("pointerlockerror", b => this._OnPointerLockError())
    }
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    _OnNavigate(a) {
        ...
        if ("back" === b)
            ...
            const c = a.target;
            a = a.exportType;
            self.cordova && self.cordova.InAppBrowser ? self.cordova.InAppBrowser.open(b, "_system") : "preview" === a || "windows-webview2" === a ? window.open(b, "_blank") : this._isConstructArcade || (2 === c ? window.top.location = b : 1 === c ? window.parent.location = b : window.location = b)
        } else
            "new-window" === b && (b = a.url,
            a = a.tag,
            self.cordova && self.cordova.InAppBrowser ? self.cordova.InAppBrowser.open(b, "_system") : window.open(b, a))
    }
    _OnRequestFullscreen(a) {
        if ("windows-webview2" === this._exportType || "macos-wkwebview" === this._exportType)
    ...
    _OnSetDocumentCSSStyle(a) {
        ...
        try {
            ...
            for (const f of e)
                b.startsWith("--") ? f.style.setProperty(b, c) : f.style[b] = c
        } catch (e) {
    ...
    _OnGetDocumentCSSStyle(a) {
        ...
        try {
            ...
            return console.warn("[Browser] Failed to get style: ", c),
            {
                isOk: !1
            }
        }
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    constructor(a) {
        ...
        this._finishedLoadingSent = this._gameplayActive = this._debugModeActive = this._pokiSDKLoaded = !1;
        this.AddRuntimeMessageHandlers([["InitPoki", this.InitPoki.bind(this)], ["NotifyGameplayStart", this.NotifyGameplayStart.bind(this)], ["NotifyGameplayStop", this.NotifyGameplayStop.bind(this)], ["HappyTime", this.HappyTime.bind(this)], ["RequestCommercialBreak", this.RequestCommercialBreak.bind(this)], ["RequestRewardedBreak", this.RequestRewardedBreak.bind(this)], ["SetDebugMode", this.SetDebugMode.bind(this)], ["GameLoadingFinished", this.GameLoadingFinished.bind(this)]])
    }
    GameLoadingFinished() {
        this._pokiSDKLoaded && !this._finishedLoadingSent && (this._finishedLoadingSent = !0,
        PokiSDK.gameLoadingFinished())
    }
    NotifyGameplayStart() {
        this._pokiSDKLoaded && (this._finishedLoadingSent || console.log("Don't forget to send the Loading Finished Notification, or configure the Poki Plugin to manage it automatically."),
        this._gameplayActive = !0,
        PokiSDK.gameplayStart())
    }
    NotifyGameplayStop() {
        this._pokiSDKLoaded && (this._gameplayActive = !1,
        PokiSDK.gameplayStop())
    }
    HappyTime({intensity: a}) {
        this._pokiSDKLoaded && PokiSDK.happyTime(a)
    }
    RequestCommercialBreak() {
        if (!this._pokiSDKLoaded)
            return {
                result: !1,
                err: !1
            };
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    _GetSafeAreaInset() {
        var a = document.body;
        const b = a.style;
        b.setProperty("--temp-sai-top", "env(safe-area-inset-top)");
        b.setProperty("--temp-sai-right", "env(safe-area-inset-right)");
        b.setProperty("--temp-sai-bottom", "env(safe-area-inset-bottom)");
        b.setProperty("--temp-sai-left", "env(safe-area-inset-left)");
        a = getComputedStyle(a);
        a = [a.getPropertyValue("--temp-sai-top"), a.getPropertyValue("--temp-sai-right"), a.getPropertyValue("--temp-sai-bottom"), a.getPropertyValue("--temp-sai-left")].map(c => {
            c = parseInt(c, 10);
            return isFinite(c) ? c : 0
        }
        );
        b.removeProperty("--temp-sai-top");
        b.removeProperty("--temp-sai-right");
        b.removeProperty("--temp-sai-bottom");
        b.removeProperty("--temp-sai-left");
        return a
    }
    _OnResize() {
        this.PostToRuntime("window-resize", {
